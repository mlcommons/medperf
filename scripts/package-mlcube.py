...
def get_args():
    ...
    parser.add_argument(
        ...
        'data-preparator' for a data preparation MLCube, 'model' for a model MLCube, and 'metrics' for a metrics MLCube.""",
    )
    parser.add_argument(
        "--output",
    ...
    if not set(args.mlcube_types).issubset(["data-preparator", "model", "metrics"]):
