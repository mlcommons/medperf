...
def load_data(data_id):
    ...
    print(
        "# train data: {0}, # test data: {1}".format(
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(trainloader):
            ...
            if i % 200 == 199:  # print every 1000 mini-batches
                ...
                end_time = time.time()
                print(
                    "Training time for 200 mini-batch: {0}".format(
                        end_time - start_time
                    )
                )
                start_time = time.time()
...
def main():
    ...
    loss, dice_total_avg, dice_class0_avg, dice_class1_avg = test(
        net=net, criterion=criterion, testloader=testloader, device=device
    )
