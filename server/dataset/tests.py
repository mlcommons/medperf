...
class DatasetTest(MedPerfTest):
    ...
    def setUp(self):
        ...
        response = self.client.post("/api/v1/mlcubes/", data_preproc_mlcube, format="json")
    ...
    def test_unauthenticated_user(self):
        ...
        response = client.delete("/api/v1/datasets/1/")
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
        response = client.put("/api/v1/datasets/1/")
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_crud_user(self):
        ...
        response = self.client.post("/api/v1/datasets/", testdataset, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        uid = response.data["id"]
        response = self.client.get("/api/v1/datasets/{0}/".format(uid))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        response = self.client.put(
            "/api/v1/datasets/{0}/".format(uid), newtestdataset, format="json"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        response = self.client.get("/api/v1/datasets/{0}/".format(uid))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        # response = self.client.delete("/api/v1/datasets/{0}/".format(uid))
    ...
    def test_invalid_dataset(self):
        invalid_id = 9999
        response = self.client.get("/api/v1/datasets/{0}/".format(invalid_id))
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
