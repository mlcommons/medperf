...
def get_col_model_paths(model_folder):
    return {
        "initial_model_path": os.path.join(
            model_folder, "model_initial_checkpoint.model"
        ),
        "final_model_path": os.path.join(model_folder, "model_final_checkpoint.model"),
        "initial_model_info_path": os.path.join(
            model_folder, "model_initial_checkpoint.model.pkl"
        ),
        "final_model_info_path": os.path.join(
            model_folder, "model_final_checkpoint.model.pkl"
        ),
    }
...
def get_col_plans_path(network, task, plans_identifier):
    ...
    plans_write_path_2d = os.path.join(
        plans_write_dirpath, plans_identifier + "_plans_2D.pkl"
    )
    plans_write_path_3d = os.path.join(
        plans_write_dirpath, plans_identifier + "_plans_3D.pkl"
    )
    ...
    if network == "2d":
        ...
        if os.path.exists(plan_dirpath):
            ...
            data_dir_2d = os.path.join(
                plan_dirpath,
                plan_2d["data_identifier"]
                + "_stage"
                + str(list(plan_2d["plans_per_stage"].keys())[-1]),
            )
...
def trim_data_and_setup_model(
    ...
    col_paths.update(
        get_col_plans_path(
            network=network, task=task, plans_identifier=plans_identifier
        )
    )
    ...
    if not init_model_path:
        if plans_path:
            raise ValueError(
                f"If the initial model is not provided then we do not expect the plans_path to be provided either (plans file and initial model are sourced the same way)."
            )
        ...
        shutil.copyfile(
            src=col_paths["final_model_path"], dst=col_paths["initial_model_path"]
        )
        shutil.copyfile(
            src=col_paths["final_model_info_path"],
            dst=col_paths["initial_model_info_path"],
        )
    else:
        ...
        shutil.copyfile(
            src=init_model_info_path, dst=col_paths["initial_model_info_path"]
        )
        # now copy these files also into the final paths
        shutil.copyfile(
            src=col_paths["initial_model_path"], dst=col_paths["final_model_path"]
        )
        shutil.copyfile(
            src=col_paths["initial_model_info_path"],
            dst=col_paths["final_model_info_path"],
        )
