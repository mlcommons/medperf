...
class SanityChecks:
    ...
    def run(self):
        ...
        csv_files = list(
            map(lambda csvname: os.path.join(csv_path, csvname), csv_files)
        )
        ...
        assert all(
            map(lambda file: get_file_extention(file) == ".csv", csv_files)
        ), "csv data folder contains non-csv files"

        assert set(map(get_file_basename, csv_files)).issubset(
            map(get_file_basename, videos)
            ...
            with open(csv_file) as read_file:
                ...
                for line in read_file.readlines():
                    if not frame:  # header line
                        ...
                        try:
                            ...
                            raise AssertionError(
                                "csv files are supposed to have two columns seperated by a comma"
                            )

                        assert (
                            not os.path.exists(
                                os.path.join(self.data_path, header1.strip())
                            )
                            or header2 not in accepted_labels
                    ...
                    assert os.path.exists(
                        frame_path
                    ), f"{frame_path}: file doesn't exist"
                    assert (
                        get_file_extention(frame_path) == ".png"
                    ), f"frames should be .png"
                    # TODO: assert frames have correct height and width
                    assert (
                        os.path.split(frame_path)[0] in videos
                    ), f"csv files try to read frames from {os.path.split(frame_path)[0]}"
                    assert (
                        label in accepted_labels
                    ), f"labels are supposed to be integers between 0 and {num_labels}"
