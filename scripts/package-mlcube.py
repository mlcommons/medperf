...
def get_args():
    ...
    parser.add_argument(
        "--mlcube",
        help="Path to the mlcube manifest file.",
    )
    parser.add_argument(
        "--mlcube-types",
        help="""Comma-separated list of mlcube types to look for. Valid ones:
...
def main():
    args = get_args()
    mlcube_manifest_path = args.mlcube
    output_path = args.output
    mlcube_types = args.mlcube_types

    required_files = validate_and_parse_manifest(mlcube_manifest_path, mlcube_types)
    package(mlcube_manifest_path, required_files, output_path)
