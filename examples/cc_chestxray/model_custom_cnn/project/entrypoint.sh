...
python mlcube.py --data_path $DATA_FOLDER/data --output_path /predictions --parameters_file /mlcube_project/parameters.yaml --weights /mlcube_project/cnn_weights.pth

tar -czvf /predictions.tar.gz /predictions
mkdir /predictions_hosted
mv /predictions.tar.gz /predictions_hosted

cd /predictions_hosted && python -m http.server --bind 127.0.0.1 5505
