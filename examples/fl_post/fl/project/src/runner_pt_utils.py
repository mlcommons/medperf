...
def rebuild_model_util(
    runner_class, input_tensor_dict, testing_with_opt_setting=False, **kwargs
):
    """
...
def expand_derived_opt_state_dict(derived_opt_state_dict, device):
    ...
    if len(derived_opt_state_dict) != 0:
        raise ValueError(
            f"Opt state should have been exausted, but we have left: {derived_opt_state_dict}"
        )
