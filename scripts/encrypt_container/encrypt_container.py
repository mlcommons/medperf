...
def generate_key_and_encrypt(
    ...
    docker_image_name: str = typer.Option(
        ...
        '--docker-image',
        help=(
            'Name (including tag) of Docker image to encrypt.'
    ...
    """
    if (
        (docker_image_name is None and singularity_image_file is None) or
        (docker_image_name is not None and singularity_image_file is not None)
        ):
        raise ValueError("Exactly one of 'docker_image_name' or 'singularity_image_file' MUST be provided!")
