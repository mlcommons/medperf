...
def process_tool_output(
    intermediate_folder, fusion, segmentation_labels_folder, seg_csv
):
    # collect tool output
    tool_outputs_dict = {}
    pattern = r".*(\d{5}-\d{3})$"
    for subj in os.listdir(intermediate_folder):
        ...
        folder = os.path.join(intermediate_folder, subj, "SegmentationsForQC")
        for file in os.listdir(folder):
            file_path = os.path.join(folder, file)
            if os.path.isdir(file_path):
                continue
            if fusion.lower() in file.lower():
                tool_outputs_dict[subj_id] = file_path
                break
