...
from .runner_pt_utils import (
    rebuild_model_util,
    derive_opt_state_dict,
...
class PyTorchCheckpointTaskRunner(TaskRunner):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if device not in ["cpu", "cuda"]:
            raise ValueError(
                "Device argument must be 'cpu' or 'cuda', but {device} was used instead."
            )
        self.device = device
        ...
        if self.device == "cuda" and not self.gpu_num_string:
            raise ValueError(
                f"If device is 'cuda' then gpu_num must be set rather than allowing to be the default None."
            )
        else:
    ...
    def rebuild_model(self, input_tensor_dict, **kwargs):
        rebuild_model_util(
            runner_class=self, input_tensor_dict=input_tensor_dict, **kwargs
        )
    ...
    def reset_opt_vars(self):
        ...
        initial_checkpoint_dict = self.load_checkpoint(
            checkpoint_path=self.checkpoint_path_initial
        )
        derived_opt_state_dict = self._get_optimizer_state(
            checkpoint_dict=initial_checkpoint_dict
        )
        self._set_optimizer_state(
            derived_opt_state_dict=derived_opt_state_dict,
    ...
    def set_tensor_dict(self, tensor_dict, with_opt_vars=False):
        ...
        """
        return self.write_tensors_into_checkpoint(
            tensor_dict=tensor_dict, with_opt_vars=with_opt_vars
        )
    ...
    def _set_optimizer_state(self, derived_opt_state_dict, checkpoint_dict):
        ...
        """
        self._write_optimizer_state_into_checkpoint(
            derived_opt_state_dict=derived_opt_state_dict,
            checkpoint_dict=checkpoint_dict,
            checkpoint_path=self.checkpoint_out_path,
        )

    def _write_optimizer_state_into_checkpoint(
        self, derived_opt_state_dict, checkpoint_dict, checkpoint_path
    ):
        ...
        temp_state_dict = expand_derived_opt_state_dict(
            derived_opt_state_dict, device=self.device
        )
        ...
        for group_idx, group in enumerate(temp_state_dict["param_groups"]):
            checkpoint_dict["optimizer_state_dict"]["param_groups"][group_idx][
                "params"
            ] = group["params"]
    ...
    def convert_results_to_tensorkeys(
        self, col_name, round_num, metrics, insert_model, add_metrics_tags=[]
    ):
        ...
        output_metric_dict = {
            TensorKey(metric_name, origin, round_num, True, metric_dict_tags): np.array(
                metrics[metric_name]
            )
