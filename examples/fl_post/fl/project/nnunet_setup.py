...
def main(
    ...
    if str(three_digit_task_num)[0] != "5":
        raise ValueError(
            f"The three digit task number: {three_digit_task_num} should start with 5 to avoid NNUnet repository tasks, but it starts with {three_digit_task_num[0]}"
        )
    if init_model_path or init_model_info_path:
        if not init_model_path or not init_model_info_path:
            raise ValueError(
                f"If either init_model_path or init_model_info_path are provided, they both must be."
            )
    if init_model_path:
    ...
    col_paths = setup_fl_data(
        postopp_pardir=postopp_pardir,
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "--postopp_pardir", type=str, help="Parent directory to postopp data."
    )
    argparser.add_argument(
        "--three_digit_task_num",
        type=int,
    ...
    argparser.add_argument(
        "--network", type=str, default="3d_fullres", help="NNUnet network to be used."
    )
    argparser.add_argument(
        "--network_trainer",
        type=str,
    ...
    argparser.add_argument(
        "--verbose", action="store_true", help="Print debuging information."
    )
