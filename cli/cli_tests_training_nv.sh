...
wget -P $DIRECTORY https://storage.googleapis.com/medperf-storage/nvflare_training/uncol1.tar.gz
tar -xf $DIRECTORY/uncol1.tar.gz -C $DIRECTORY
wget -P $DIRECTORY https://storage.googleapis.com/medperf-storage/nvflare_training/uncol2.tar.gz
tar -xf $DIRECTORY/uncol2.tar.gz -C $DIRECTORY

rm $DIRECTORY/uncol1.tar.gz
rm $DIRECTORY/uncol2.tar.gz
...
print_eval medperf container submit --name trainprep -m $NV_PREP_TRAINING_MLCUBE -p $NV_PREP_TRAINING_PARAMS --operational
checkFailed "Train prep submission failed"
PREP_UID=$(medperf container ls | grep trainprep | head -n 1 | tr -s ' ' | cut -d ' ' -f 2)

print_eval medperf container submit --name traincube -m $NV_TRAIN_MLCUBE --operational
checkFailed "fl container submission failed"
TRAINCUBE_UID=$(medperf container ls | grep traincube | head -n 1 | tr -s ' ' | cut -d ' ' -f 2)

print_eval medperf container submit --name fladmincube -m $NV_FLADMIN_MLCUBE --operational
...
print_eval medperf aggregator submit -n aggreg -a $HOSTNAME_ -p 8102 -p 8103 -m $TRAINCUBE_UID
...
echo "submit plan"
echo "====================================="
print_eval medperf training set_plan -t $TRAINING_UID -c $NV_TRAINING_CONFIG -y
...
echo "====================================="
print_eval medperf dataset submit -p $PREP_UID -d $DIRECTORY/uncol1/data -l $DIRECTORY/uncol1/labels --name="col1" --description="col1data" --location="col1location" -y
...
echo "====================================="
print_eval medperf dataset submit -p $PREP_UID -d $DIRECTORY/uncol2/data -l $DIRECTORY/uncol2/labels --name="col2" --description="col2data" --location="col2location" -y
...
sleep 120 # sleep some time
echo "status: Done" >>./report.yaml

print_eval medperf training close_event -t $TRAINING_UID -r ./report.yaml -y
