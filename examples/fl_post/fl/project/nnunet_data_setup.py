...
def create_task_folders(task_num, task_name, overwrite_nnunet_datadirs):
    ...
    nnunet_dst_pardir = os.path.join(
        os.environ["nnUNet_raw_data_base"], "nnUNet_raw_data", f"{task}"
    )
    ...
    task_cropped_pardir = os.path.join(
        os.environ["nnUNet_raw_data_base"], "nnUNet_cropped_data", f"{task}"
    )
    task_preprocessed_pardir = os.path.join(
        os.environ["nnUNet_raw_data_base"], "nnUNet_preprocessed", f"{task}"
    )

    if not overwrite_nnunet_datadirs:
        if os.path.exists(nnunet_images_train_pardir) and os.path.exists(
            nnunet_labels_train_pardir
        ):
            raise ValueError(
                f"Train images pardirs: {nnunet_images_train_pardir} and {nnunet_labels_train_pardir} both already exist. Please move them both and rerun to prevent overwriting."
            )
        elif os.path.exists(nnunet_images_train_pardir):
            raise ValueError(
                f"Train images pardir: {nnunet_images_train_pardir} already exists, please move and run again to prevent overwriting."
            )
        elif os.path.exists(nnunet_labels_train_pardir):
            raise ValueError(
                f"Train labels pardir: {nnunet_labels_train_pardir} already exists, please move and run again to prevent overwriting."
            )

        if os.path.exists(task_cropped_pardir):
            raise ValueError(
                f"Cropped data pardir: {task_cropped_pardir} already exists, please move and run again to prevent overwriting."
            )
        if os.path.exists(task_preprocessed_pardir):
            raise ValueError(
                f"Preprocessed data pardir: {task_preprocessed_pardir} already exists, please move and run again to prevent overwriting."
            )
    else:
        if os.path.exists(task_cropped_pardir):
...
def symlink_one_subject(
    ...
    if timestamp_selection == "latest":
        ...
        raise ValueError(
            f"timestamp_selection currently only supports 'latest', 'earliest', and 'all', but you have requested: '{timestamp_selection}'"
        )
    ...
    for timestamp in timestamps:
        ...
        if not os.path.exists(postopp_subject_timestamp_label_dirpath):
            raise ValueError(
                f"Subject label file for data at: {postopp_subject_timestamp_dirpath} was not found in the expected location: {postopp_subject_timestamp_label_dirpath}"
            )
        ...
        label_dst_path = os.path.join(
            nnunet_labels_train_pardir, timed_subject + ".nii.gz"
        )
        ...
        for num in num_to_modality:
            ...
            dst_path = os.path.join(
                nnunet_images_train_pardir, timed_subject + num + ".nii.gz"
            )
...
def doublecheck_postopp_pardir(postopp_pardir, verbose=False):
    ...
    if "data" not in postopp_subdirs:
        raise ValueError(
            f"'data' must be a subdirectory of postopp_src_pardir:{postopp_pardir}, but it is not."
        )
    if "labels" not in postopp_subdirs:
        raise ValueError(
            f"'labels' must be a subdirectory of postopp_src_pardir:{postopp_pardir}, but it is not."
        )
...
def split_by_subject(subject_to_timestamps, percent_train, split_seed, verbose=False):
    ...
    train_subject_to_timestamps = {
        subject: subject_to_timestamps[subject] for subject in subjects[:train_cutoff]
    }
    val_subject_to_timestamps = {
        subject: subject_to_timestamps[subject] for subject in subjects[train_cutoff:]
    }
...
def split_by_timed_subjects(
    subject_to_timestamps, percent_train, split_seed, random_tries=30, verbose=False
):
    """
    ...
    def shuffle_and_cut(
        subject_counts, grand_total, percent_train, seed, verbose=False
    ):
        ...
        for idx in range(2, len(subjects) + 1):
            ...
            percent_train_estimate = percent_train_for_split(
                train_subjects=train_subjects, grand_total=grand_total
            )
    ...
    subject_counts = {
        subject: len(subject_to_timestamps[subject])
        for subject in subject_to_timestamps
    }
    ...
    best_percent_train = percent_train_for_split(
        train_subjects=best_train_subjects, grand_total=grand_total
    )
    ...
    for _try in range(random_tries):
        ...
        train_subjects, val_subjects, percent_train_estimate = shuffle_and_cut(
        ...
        if abs(percent_train_estimate - percent_train) < abs(
            best_percent_train - percent_train
        ):
    ...
    train_subject_to_timestamps = {
        subject: subject_to_timestamps[subject] for subject in best_train_subjects
    }
    val_subject_to_timestamps = {
        subject: subject_to_timestamps[subject] for subject in best_val_subjects
    }
    return train_subject_to_timestamps, val_subject_to_timestamps


def write_splits_file(
    ...
    fold,
    task,
    splits_fname="splits_final.pkl",
    verbose=False,
):
    # double check we are in the right folder to modify the splits file
    splits_fpath = os.path.join(
        os.environ["nnUNet_preprocessed"], f"{task}", splits_fname
    )
    POSTOPP_splits_fpath = os.path.join(
        os.environ["nnUNet_preprocessed"], f"{task}", "POSTOPP_BACKUP_" + splits_fname
    )

    # now split
    if split_logic == "by_subject":
        train_subject_to_timestamps, val_subject_to_timestamps = split_by_subject(
        ...
        train_subject_to_timestamps, val_subject_to_timestamps = (
            split_by_timed_subjects(
                subject_to_timestamps=subject_to_timestamps,
                percent_train=percent_train,
                split_seed=split_seed,
                verbose=verbose,
        ...
        raise ValueError(
            f"Split logic of 'by_subject' and 'by_subject_time_pair' are the only ones supported, whereas a split_logic value of {split_logic} was provided."
        )
    ...
    new_folds[int(fold)] = OrderedDict(
        {"train": np.array(train_subjects_list), "val": np.array(val_subjects_list)}
    )
...
def setup_fl_data(
    ...
    for postopp_subject_dir in all_subjects:
        subject_to_timestamps[postopp_subject_dir] = symlink_one_subject(
            postopp_subject_dir=postopp_subject_dir,
            postopp_data_dirpath=postopp_data_dirpath,
    ...
    (
        ...
        col_paths["plans_path"],
    ) = trim_data_and_setup_model(
        task=task,
        network=network,
