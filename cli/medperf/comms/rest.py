...
class REST(Comms):
    ...
    def get_kbs(self, kbs_uid: int) -> dict:
        url = f"{self.server_url}/kbss/{kbs_uid}/"
        error_msg = "Could not retrieve kbs"
        return self.__get(url, error_msg)
    ...
    def upload_kbs(self, kbs_dict: dict) -> int:
        url = f"{self.server_url}/kbss/"
        error_msg = "could not upload kbs"
        return self.__post(url, json=kbs_dict, error_msg=error_msg)
