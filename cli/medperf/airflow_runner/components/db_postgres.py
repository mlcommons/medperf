...
class PostgresDBDocker(DatabaseComponent):
    ...
    def __init__(self, project_name: str, root_dir: os.PathLike,
                 postgres_user: str = 'postgres', postgres_password: SecretStr = None, 
                 postgres_db: str = 'postgres', postgres_port: Union[str, int] = 5432,
    ...
    async def start_logic(self):
        self.process = subprocess.run(
            [
                ...
                '-e', f'POSTGRES_PASSWORD={self.password.get_secret_value()}',
                '-e', f'POSTGRES_DB={self.db}',
    ...
    def has_successfully_started(self):
        postgres_status: subprocess.CompletedProcess = subprocess.run(
            [
                'docker', 'exec', '-it', self.container_id,
                "pg_isready", "-U", self.user, "-d", self.db
            ], capture_output=True, text=True
        )
        return postgres_status.returncode == 0

    def terminate(self):
        subprocess.run(['docker', 'stop', self.container_id], capture_output=True, text=True)
        subprocess.run(['docker', 'rm', self.container_id], capture_output=True, text=True)
