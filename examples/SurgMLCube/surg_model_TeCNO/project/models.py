...
class DilatedResidualLayer(tf.keras.models.Model):
    def __init__(self, dilation, out_channels, dropout_rate=0.5, kernel_size=3):
        ...
        self.conv_dilated = tf.keras.layers.Conv1D(filters=out_channels,
...
class SingleStageModel(tf.keras.models.Model):
    def __init__(self,
        ...
        self.layers_list = [DilatedResidualLayer(2**i, num_f_maps) for i in range(num_layers)]
...
class MultiStageModel(tf.keras.models.Model):
    def __init__(self, num_stages, num_layers, num_f_maps, num_classes):
        ...
        self.stage1 = SingleStageModel(num_layers,
                                        num_f_maps,
                                        num_classes) 
        self.stages = [SingleStageModel(num_layers,
                                        num_f_maps,
                                        num_classes)
