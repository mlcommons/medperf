...
class PyTorchNNUNetCheckpointTaskRunner(PyTorchCheckpointTaskRunner):
    ...
    def __init__(
        self, nnunet_task=None, config_path=None, actual_max_num_epochs=1000, **kwargs
    ):
        """Initialize.
        ...
        if "nnUNet_raw_data_base" not in os.environ:
            raise ValueError(
                "NNUNet V1 requires that 'nnUNet_raw_data_base' be set either in the flplan or in the environment variables"
            )
        if "nnUNet_preprocessed" not in os.environ:
            raise ValueError(
                "NNUNet V1 requires that 'nnUNet_preprocessed' be set either in the flplan or in the environment variables"
            )
        if "RESULTS_FOLDER" not in os.environ:
            raise ValueError(
                "NNUNet V1 requires that 'RESULTS_FOLDER' be set either in the flplan or in the environment variables"
            )
        ...
        self.task_completed = {
            "aggregated_model_validation": 1.0,
            "train": 1.0,
    ...
    def write_tensors_into_checkpoint(self, tensor_dict, with_opt_vars):
        ...
        checkpoint_dict = self.load_checkpoint(
            checkpoint_path=self.checkpoint_path_load, map_location=device
        )
        ...
        if with_opt_vars:
            ...
            if tensor_dict.pop("__opt_state_needed") == "true":
                checkpoint_dict = self._set_optimizer_state(
                    derived_opt_state_dict=tensor_dict, checkpoint_dict=checkpoint_dict
                )
    ...
    def train(
        ...
        input_tensor_dict,
        epochs,
        val_cutoff_time=np.inf,
        train_cutoff_time=np.inf,
        train_completion_dampener=0.0,
        **kwargs,
    ):
        ...
        (
            ...
            actual_max_num_epochs=self.actual_max_num_epochs,
            fl_round=round_num,
        ...
        global_tensor_dict, local_tensor_dict = self.convert_results_to_tensorkeys(
            col_name, round_num, metrics, insert_model=True
        )

        self.logger.info(
            f"Completed train/val with {int(train_completed*100)}% of the train work and {int(val_completed*100)}% of the val work. Exact rates are: {train_completed} and {val_completed}"
        )
        ...
        **kwargs,
        ...
        if not from_checkpoint:
            ...
            if train_completed != 0.0:
                raise ValueError(
                    f"Tried to validate only, but got a non-zero amount ({train_completed}) of training done."
                )
            ...
            # 3. Prepare metrics
            metrics = {
                "val_eval": this_val_eval_metrics,
            ...
            (
                ...
                all_val_eval_metrics_C4,
            ) = checkpoint_dict["plot_stuff"]
        ...
        return self.convert_results_to_tensorkeys(
            col_name,
            round_num,
            metrics,
            insert_model=False,
            add_metrics_tags=add_metrics_tags,
        )
    ...
    def get_train_data_size(self, task_name=None):
        ...
        if not task_name:
            ...
            return int(
                np.ceil(
                    self.task_completed[task_name] ** (-1)
    ...
    def get_valid_data_size(self, task_name=None):
        ...
        if not task_name:
            ...
            return int(
                np.ceil(
                    self.task_completed[task_name] ** (-1)
                    * self.data_loader.get_valid_data_size()
                )
            )
